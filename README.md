...
- **修复:非默认语言界面回复后全部内容变为锁定**。本地化 cooked 的容器指纹对应
  ...
  现注入器会依次尝试各语言本地化的 raw 对齐,对齐成功后按用户权限逐块处理:
  未解锁用户看到本语言占位框,已解锁用户看到本语言翻译内容
- **修复:本地化变体被一刀切拦截**。v1.1.2 对含标记帖子的本地化变体一律拒绝
  （非默认语言用户回退中文默认版）。现仅当翻译产物丢失容器结构
  （翻译内容明文暴露的破损翻译）时才拒绝,结构完好的变体直接放行,
  不再影响正常翻译功能
- 新增教程链接;测试 79 → 81 个
