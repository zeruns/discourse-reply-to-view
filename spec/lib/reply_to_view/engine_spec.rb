...
RSpec.describe ReplyToView::Engine do
  describe ".extract" do
    ...
    it "提取 [reply=N] 的计数值" do
      blocks = described_class.extract("[reply=5]\nSECRET\n[/reply]\n")
      expect(blocks[0].count).to eq(5)
    end

    it "非法计数值返回 nil（0、负数、非数字）" do
      expect(described_class.extract("[reply=0]\nx\n[/reply]\n")[0].count).to be_nil
      expect(described_class.extract("[reply=-1]\nx\n[/reply]\n")[0].count).to be_nil
      expect(described_class.extract("[reply=abc]\nx\n[/reply]\n")[0].count).to be_nil
    end
    ...
    it "未闭合的标记不产生块" do
      expect(described_class.extract("[reply]\nSECRET\n")).to be_empty
      expect(described_class.extract("前置 [reply]\nSECRET\n[/reply]\n")).to be_empty
    end

    it "内容保留原始缩进" do
      blocks = described_class.extract("[reply]\n  indented\n    deeper\n[/reply]\n")
